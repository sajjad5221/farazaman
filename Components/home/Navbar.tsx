...
export default function Navbar({ lang }: { lang: string }) {
  ...
  return (
    <Disclosure
      as="nav"
      className="drawer fixed top-0 left-0 right-0 bg-white   text-neutral-900 border-b border-neutral-200 backdrop-blur-sm"
    >
      {({ open }: { open: boolean }) => (
        <>
          {/* Desktop navbar */}
          ...
          {/* <Disclosure.Panel className="sm:hidden">
            <div className="space-y-1 px-4 min-h-screen border-t border-neutral-200   bg-white  ">
              {navigation.map((item) => (
                <Disclosure.Button key={item.name} to={item.goTo}>
                  {item.name}
                </Disclosure.Button>
              ))}
            </div>
