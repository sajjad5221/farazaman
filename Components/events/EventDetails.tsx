...
export default function EventDetails({ events, eventId }: Props) {
  // set the maximum length of the event description that can be shown
  const MAX_DESCRIPTION_LENGTH = 650;
  // use state to toggle whether to show the full event description or not
  const [showFullDescription, setShowFullDescription] = useState(false);
  // decrement the eventId by one to match the array index
  eventId = eventId - 1;
  // get the event object matching the given eventId
        ...
        <div className="grid grid-cols-4 mb-10 ">
          ...
          <div className="order-2 col-span-4 py-0 text-right text-gray-900 md:px-10 dark:text-gray-100 lg:mx-16 px-7 lg:col-span-2 ">
            ...
            <p
              ...
              id="description"
            >
              {/* show a truncated version of the description if it exceeds the maximum length */}
              {event?.description.length > MAX_DESCRIPTION_LENGTH &&
              !showFullDescription
                ? event?.description.slice(0, MAX_DESCRIPTION_LENGTH)
                : event?.description}
              {/* show a "less" link to toggle between the truncated and full description */}
              {event?.description.length > MAX_DESCRIPTION_LENGTH && (
                <span>
                  {showFullDescription && (
                    <>
